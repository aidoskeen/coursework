...
void student::student_register() {
	...
	case 2:strcpy_s(room_type, "Double"); room_cost = 110;
		...
		while (room_num < 2 && room_num>50 && room_num ==11 && room_num ==21 && room_num == 31 && room_num ==41) {
			cout << "\n\aYou entered invalid room number. Enter correct room number:";
			cin >> room_num;
		}
		 break;
	default:break;
...
void show_info() {
	...
	cout << "\nIf some data is not proper,you can correct it.\nIf you want to do so, please type 1. If you want to go back to main menu type 2.\n";
	cin >> n;
	switch (n) {
...
void leave_dorm()
{
	...
	{
		if (s.id_ret() == n)
			file_2.write((char*)&s, sizeof(student));
		else flag = true;
	}
	...
	if (flag)
		cout << "\n\n\tYou succesflly logged out from dormitory system.";
	else
